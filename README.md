...
```Bash
cd pyplusplus11/ # Go to root of directory
source env.bashrc
```
...
One can decides to execute the C++ tests or not through Python by changing in env.bashrc the variable `PYPLUSPLUS11_PYTEST_RUN_CPP_TESTS` to `True` or `False`.
...
We put our virtualenvs in  `~/virtualenvs`. In this folder we can create a virtualenv which will be used for installing and running pyplusplus11

```Bash
## Equivalent to pypp_venv
# Create virtual environment
mkdir ~/virtualenvs
virtualenv ~/virtualenvs/pyplusplus11
# Run the virtual environment, isolated from the hosting computer
source ~/virtualenvs/pyplusplus11/bin/activate # equivalent to pypp_activate

# Do what you want with seeps, here we just run a python interpreter and ensure we can import the software
ipyton
import pyplusplus11
...
The archive will be created in $PYPLUSPLUS11_HOME/dist/ folder
...
To add a new feature to pyplusplus11, one must understand how it is built:

First, pyplusplus11 is constructed as a big python package with subpackages where functionnalities come either from python functions or from C++ functions.
...
3. Create the Python/C++ module and add it as a submodule of the full seeps package
...
1. Create your C++ module file and put it in the right place in pyplusplus11 file's architecture. Add the .hpp file in `include/` folder
...
###### Step 3: Create the Python/C++ module and add it as a submodule of the full pyplusplus11 package

Expose the newly pybind python module through the Python module in pyplusplus11
